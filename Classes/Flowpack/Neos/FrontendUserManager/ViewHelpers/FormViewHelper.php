...
class FormViewHelper extends AbstractFormViewHelper {
	...
	protected function getFormActionUri() {
		...
		if ($this->hasArgument('actionUri')) {
			if ($this->arguments['actionUri'] === 'self') {
				$request = $this->controllerContext->getRequest();
				$argumentNamespace = $request->getArgumentNamespace();
				$arguments['@action'] = strtolower($this->arguments['action']);
				$arguments['@controller'] = strtolower($request->getControllerName());
				$arguments['@package'] = strtolower($request->getControllerPackageKey());
				if ($argumentNamespace !== '') {
					$arguments = array($argumentNamespace => $arguments);
				}
				$uri = $this->helperService->getUriFromRequest($request);
				$this->formActionUri = $uri . '?' . http_build_query($arguments, NULL, '&');
			} else {
